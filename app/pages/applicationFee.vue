...
<header class="w-full flex flex-col gap-1 items-start">
    ...
    <main class="w-full h-screen flex justify-center">
        <div class="h-fit bg-gray-200 p-10 rounded-2xl shadow-xl flex flex-col gap-5">
            ...
            <form @submit.prevent="pay" class="flex flex-col gap-2">
...
const props = ref({
    ...
    regNum: '',
    amount: 5_000, // amount 
    refSalt: new Date().getSeconds()+''+Math.floor((Math.random() * 1_000_000_000) + 1) * Math.floor((Math.random() * 1_000_000_000) + 1),
});
...
const pay = () => {
    ...
    const handler = PaystackPop.setup({
        ...
        ref: `ref-${props.value.refSalt}`, // Replace with a reference you generated
